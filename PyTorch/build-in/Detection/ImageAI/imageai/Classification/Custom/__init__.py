                ...
                for imgs, labels in tqdm(self.__data_loaders[phase]):
                    ...
                    with torch.set_grad_enabled(phase == "train"):
                        ...
                        if phase=="train":
                            ...
                            torch.nn.utils.clip_grad_norm_(self.__model.parameters(), max_norm=0.1)
                            #loss.backward()
                            self.__optimizer.step()

                    # if idx % 50 == 0:
                    #     logging.info(f"epoch: {epoch}, batch: {idx}")
                    #     max_abs_values = [i.abs().max().item() for i in self.__model.parameters()]
                    #     for i in range(0, len(max_abs_values), 8):
                    #         logging.info(" ".join(f"{v:.6f}" for v in max_abs_values[i:i+8]))
