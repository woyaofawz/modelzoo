...
from efficientformer import efficientformer_l1  # 导入 EfficientFormer-L1 模型
import os
import argparse
import time
import datetime
...
def get_args():
    """解析命令行参数"""
    parser = argparse.ArgumentParser(description='Train EfficientFormer on sdaa')
...
def setup_model_and_optimizer(device, args):
    """初始化模型、损失函数、优化器和调度器"""
    model = efficientformer_l1(pretrained=args.pretrained, num_classes=1000).to(device)  # 使用 EfficientFormer-L1 模型
...
def main():
    ...
    '''
        if acc > best_acc:
            best_acc = acc
            torch.save(model.state_dict(), os.path.join(args.save_path, 'efficientformer_l1_best.pth'))
            json_logger.log(
                step=(epoch,),
    ...
    torch.save(model.state_dict(), os.path.join(args.save_path, 'efficientformer_l1_final.pth'))
    '''
    json_logger.log(
        step=(),
