...
class BracketedDataset(Dataset):
    def __init__(self, root, image_dir=None, patch_size=64, transform=None, file_num=None):
        ...
        # 确保 self.over 和 self.under 的长度一致
        if len(self.over) != len(self.under):
            min_len = min(len(self.over), len(self.under))
            self.over = self.over[:min_len]
            self.under = self.under[:min_len]
