...
def fit_one_epoch(model_train, model, loss_history, eval_callback, optimizer, epoch, epoch_step, epoch_step_val, gen, gen_val, Epoch, cuda, dice_loss, focal_loss, cls_weights, num_classes, fp16, scaler, save_period, save_dir, local_rank=0):
    ...
    for iteration, batch in enumerate(gen):
        ...
        if not fp16:
            ...
            # scaler.update()
            scaler.scale(loss).backward()  # 使用 GradScaler 缩放损失并反向传播

            # 梯度裁剪
            scaler.unscale_(optimizer)  # 将梯度反缩放为原始值
            clip_grad_norm_(model_train.parameters(), max_norm=1.0)  # 设置梯度裁剪的阈值
        ...
        if local_rank == 0:
            ...
            # 输出每个iteration的损失
            print(f"Iteration {iteration+1}/{epoch_step} - Loss: {loss.item():.4f}")
    ...
    for iteration, batch in enumerate(gen_val):
        ...
        if local_rank == 0:
            ...
            # 输出每个validation iteration的损失
            print(f"Validation Iteration {iteration+1}/{epoch_step_val} - Loss: {loss.item():.4f}")
