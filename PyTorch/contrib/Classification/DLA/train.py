...
from dla import dla34  # 导入 DLA 模型
import os
import argparse
import time
import datetime
...
def get_args():
    """解析命令行参数"""
    parser = argparse.ArgumentParser(description='Train DLA on sdaa')
    ...
    parser.add_argument('--num_steps', type=int, default=3100, help='Number of training steps')
    parser.add_argument('--pretrained', type=str, default=None, help='Pretrained model name (e.g., dla34)')
...
def setup_model_and_optimizer(device, args):
    """初始化模型、损失函数、优化器和调度器"""
    model = dla34(pretrained=args.pretrained, num_classes=1000).to(device)  # 使用 DLA-34 模型
...
def train_one_epoch(model, train_loader, criterion, optimizer, device, epoch, args):
    ...
    for i, (images, labels) in enumerate(train_loader):
        ...
        if iteration % args.num_steps == 0:
            ...
            accuracy = 100 * correct / total  # 修正为百分比
            json_logger.log(
                step=(epoch, iteration),
                data={
...
def main():
    ...
    '''   
        if acc > best_acc:
            best_acc = acc
            torch.save(model.state_dict(), os.path.join(args.save_path, 'dla34_best.pth'))
            json_logger.log(
                step=(epoch,),
    ...
    torch.save(model.state_dict(), os.path.join(args.save_path, 'dla34_final.pth'))
    '''
    json_logger.log(
        step=(),
