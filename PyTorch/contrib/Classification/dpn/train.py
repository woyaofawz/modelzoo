...
def get_args():
    """解析命令行参数"""
    parser = argparse.ArgumentParser(description='Train DPN on sdaa')
    ...
    parser.add_argument('--pretrained', type=str, default=None, help='Pretrained model name (e.g., dpn92)')
...
def setup_model_and_optimizer(device, args):
    """初始化模型、损失函数、优化器和调度器"""
    model = dpn_fast (num_classes=1000).to(device)  # 使用 DPN-92 模型
...
def main():
    ...
    '''
        if acc > best_acc:
            best_acc = acc
            torch.save(model.state_dict(), os.path.join(args.save_path, 'dpn92_best.pth'))
            json_logger.log(
                step=(epoch,),
    ...
    torch.save(model.state_dict(), os.path.join(args.save_path, 'dpn92_final.pth'))
    '''
    json_logger.log(
        step=(),
