...
class MAML:
    ...
    def calculate_per_loss(self, support_src, support_tgt, query_src, query_tgt):
        ...
        #upport_loss.backward()
        self.scaler.scale(support_loss).backward()
    ...
    def manual_gradient_update(self):
        ...
        for name, param in self.model.named_parameters():
            if param.grad is not None:
                # 由于梯度被缩放，需要除以缩放因子
                scaled_grad = param.grad / self.scaler.get_scale()
                if 'conv1' in name:
                    if 'weight' in name:
                        self.manul_params.append(param - self.sub_lr * scaled_grad)
                    elif 'bias' in name:
                        self.manul_params.append(param - self.sub_lr * scaled_grad)
                elif 'bn1' in name:
                    if 'weight' in name:
